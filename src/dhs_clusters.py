...
dask_distributed = (
    config.getboolean("main", "dask_distributed")
    if "dask_distributed" in config["main"]
    else False
)
...
def create_extract_file(output_name, data_dir):
    ...
    extract_job_text = (
        base_extract_job_path.read_text()
        .replace("[[ISO2]]", iso2)
        .replace("[[DHS_ROUND]]", output_name)
    )
...
def load_dhs_data(var_dict, data_dir):
    ...
    replacement_rules = [
        (field, rule)
        for field, value in var_dict.items()
        for rule in value.get("replacements", [])
    ]
    ...
    for s in set([i.get("source") for i in var_dict.values() if i.get("source")]):
        tmp_path = glob.glob(str(data_dir / "dhs" / "**" / f"{s}.DTA"), recursive=True)[
            0
        ]
...
def load_dhs_geo(dhs_geo_file_name, data_dir):
    ...
    shp_path = glob.glob(
        str(data_dir / "dhs" / "**" / dhs_geo_file_name / "*.shp"), recursive=True
    )[0]
