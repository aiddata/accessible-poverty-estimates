...
dhs_status_url = (
    "https://dhsprogram.com/methodology/survey-search.cfm?sendsearch=1&crt=1&listgrp=2"
)
...
status_df["start_year"] = status_df["Dates of Fieldwork"].apply(
    lambda x: int(x.split("-")[0].strip().split("/")[-1])
)
status_df["end_year"] = status_df["Dates of Fieldwork"].apply(
    lambda x: int(x.split("-")[1].strip().split("/")[-1])
)
...
status_df["country"] = status_df["Country/Year"].apply(
    lambda x: re.split("(\d+)", x)[0].strip()
)
status_df["year_label"] = status_df.apply(
    lambda x: x["Country/Year"].split(x.country)[1].split("(")[0].strip(), axis=1
)
...
status_df[["iso2", "iso3"]] = status_df.apply(
    lambda x: get_iso23(x.country), axis=1, result_type="expand"
)
...
status_df[["hr_regex", "ge_regex"]] = status_df.apply(
    lambda x: get_file_names(x), axis=1, result_type="expand"
)
...
status_df.to_csv(os.path.join(project_dir, "data", "dhs_availability.csv"), index=False)
...
status_df["survey_name"] = status_df.apply(
    lambda x: "{}_{}_DHS".format(x.iso2, x.year_label), axis=1
)
