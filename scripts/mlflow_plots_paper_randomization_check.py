...
for run in runs:
    ...
    if "version" in run_data.tags.keys():
        ...
        if run_tags["version"] not in versions:
            continue
...
# #########
parent_df = parent_df.loc[parent_df.classification.isin(["small-alt", "medium-alt", "small", "medium" "all", "hoheq-alt", "hoheq"])].copy()
parent_df = parent_df.loc[parent_df.model_name.isin(["sub-geo"])].copy()
# #########
...
stats_df = parent_df.loc[parent_df.version == "1.8.0"].copy()


small_df = stats_df.loc[stats_df.classification == "small-alt"].copy()
medium_df = stats_df.loc[stats_df.classification == "medium-alt"].copy()
male_df = stats_df.loc[stats_df.classification.isin(["hoheq-alt"]) & stats_df.gender.isin(["male"])].copy()
female_df = stats_df.loc[stats_df.classification.isin(["hoheq-alt"]) & stats_df.gender.isin(["female"])].copy()

for df in [small_df, medium_df, male_df, female_df]:
    min = round(df.r2.min(), 3)
    max = round(df.r2.max(), 3)
    mean = round(df.r2.mean(), 3)
    std = round(df.r2.std(), 3)
    range = round(max - min, 3)
    print(df.classification.unique()[0], df.gender.unique()[0])
    print("\tmin\tmax\tmean\tstd\trange")
    print(f"\t{min}\t{max}\t{mean}\t{std}\t{range}")
...
model_gender_agg = stats_df.groupby("model_and_gender", as_index=False).agg({"r2": ["min", "mean", "max", "std"]})
...
all_df_set = parent_df.loc[parent_df.version == "1.8.0"].copy()
